...
class IMG_Stegno:
    ...
    def main(self, root):
        ...
        root.config(bg='#f4e3f1')
        frame = Frame(root)
        frame.grid()

        title = Label(frame, text='Shaknoah Image Stenographer')
        title.config(font=('Helvetica', 25, 'bold'))
        title.grid(pady=10)
        title.config(bg='#6734eb')
        title.grid(row=1)

        encode = Button(frame, text="Encode", command=lambda: self.encode_frame1(frame), padx=20, bg='#e3f4f1')
        encode.config(font=('Helvetica', 14), bg='#e8c1c7')
        encode.grid(row=4)
        decode = Button(frame, text="Decode", command=lambda: self.decode_frame1(frame), padx=20, bg='#e3f4f1')
    ...
    def encode_frame1(self, F):
        ...
        label1.config(font=('', 25, 'bold'), bg='#6734eb')
        label1.grid()
